...
every day he may soon want to know how many years it was until he would 
be born, so your function needs to work with both dates in the future and in 
the past.

Provide output in this format: For dates in the future: 
"You are ... year(s) old." For dates in the past: 
"You will be born in ... year(s)." If the year of birth equals the year 
...
def calculate_age(year_of_birth: int, current_year: int) -> str:
    ...
    if year_of_birth == current_year:
        ...
        age = ['year', 'years'][(current_year - year_of_birth) > 1]
        res = f'You are {current_year - year_of_birth} {age} old.'
    else:
        age = ['year', 'years'][(year_of_birth - current_year) > 1]
        res = f'You will be born in {year_of_birth - current_year} {age}.'
    return res
