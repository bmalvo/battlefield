...
It's changing all naming conventions from camel to snake case 
(camelCasing to snake_casing). 
Every capital letter is replaced with its lowercase prefixed by 
an underscore _, except for the first letter, which is lowercased without 
...
def camel_to_snake(txt):
    """refactoring from camel to snake case"""
    res = ['_' + char.lower() if char.isupper() else char for char in txt]
    output = ''.join(res)
